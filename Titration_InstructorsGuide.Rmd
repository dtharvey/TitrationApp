---
title: "Introduction to Acid-Base Titrations: Instructor's Guide"
...
---
## Introduction
This learning module provides an introduction to acid-base titrimetry that is suitable for an introductory course in analytical chemistry. It is assumed that students are comfortable with stoichiometric calculations, particularly as they apply to the reactions of acids and bases during a titration, and that they appreciate the difference between a titration's equivalence point and its end point. The module consists of five investigations:
...
The learning module is programmed in R ([www.r-project.org](https://www.r-project.org/)) using the `Shiny` package, which allows for interactive features. Each investigation includes a brief introduction and some suggestions of things to explore and/or questions to answer. The controls, which consist of a combination of radio buttons and sliders, allow the user to choose the chemical form of the analyte and, by default, the titrant, along with their concentrations, and, where appropriate, to specify p${K}_{a}$ or p${K}_{b}$ values. Additional controls are available to adjust the limits for the volume axis and to overlay the pH range for selected indicators. The resulting titration curve is updated with each change in a control's value.

The purpose of this document is to provide instructors with additional background on the program's features and to summarize some general observations regarding each investigation. 

Two additional notes regarding the representation of indicators. First, to display an indicator's colors, the program applies a linear gradient between the color of the indicator's acid form and the color of its base form. Because the actual gradient likely is not linear and the perception of color is different between individuals, the actual colors will not match exactly the colors as displayed; nevertheless, the display is sufficient for our purposes. Second, the color gradient is set to a transparency of about 60% so that the titration curve remains visible; although this is the case when displayed on the screen or when printing from within the program, when prepared in pdf format, as is the case in this document, the indicator's color gradient is not transparent---nevertheless, the shape of the titration curve is evident. 
...
For an acid-base titration, each available function uses a single master equation to calculate the volume of titrant needed to achieve a desired pH, as outlined in Robert de Levie's *Principles of Quantitative Chemical Analysis* (McGraw-Hill, 1997). Because a function calculates the volume of titrant needed to achieve a fixed pH, the titration curve's first point can deviate significantly from a volume of zero; should this surprise students---whose experiences in lab, after all, will reasonably lead them to expect an initial pH when ${V}_\textrm{titrant}$ is 0.00 mL---a simple explanation of how the titration curves are calculated should be sufficient to alleviate any concerns. You will find additional details on the package at [CRAN](https://cran.r-project.org/web/packages/titrationCurves/index.html) or on [Github](https://github.com/dtharvey/titrationCurves).

## Investigation 1: Titration of a Strong Acid or Strong Base
Students should recognize that the three variables in this simulation---the titrant's concentration, the analyte's concentration, and the initial volume of the analyte's solution---determine the location of the titration curve's equivalence point along the *x*-axis and have little effect on the shape of the titration curve other than a small increase in $\Delta$pH for higher concentrations of strong acid and/or strong base. Of the available indicators, the most suitable across a range of concentrations are methyl red, bromothymol blue, cresol red, thymol blue (its second, more basic end point), and phenolphthalein. A typical example of a titration curve is shown below for the titration of 25.0 mL of 0.10 M strong acid using 0.10 M strong base as the titrant and bromothymol blue as an indicator.
...
plot(out, xlim = c(0, 50), ylim = c(0, 14), xaxp = c(0, 50, 10), xaxs = "i", yaxs = "i", 
     type = "l", lwd = 2, col = "blue", xlab = "volume of titrant (mL)", ylab = "pH")
...
but that only aliziran yellow R is suitable when the p${K}_{a}$ value is 9, although the width of the equivalence point along the volume axis is such that precision will suffer if we rely on a change in the indicator's color instead of recording and using the titration curve itself. 
...
and the third for the titration of the diprotic weak acid with a $\Delta$pH of 5, but with p${K}_{a}$ values of 2 and 7.
...
## Investigation 5: Practice Problems--Designing Titrations
There are many different solutions to these problems that stay within the constraints of the simulations and that use less than 50 mL of titrant. What follows are some general observations for each problem.

**Problem 1.** Although the analyte is a weak base, the titration is between the strong acid, HCl, and a strong base, NaOH. A 0.10-g sample will contain at least 0.095 g of guanine, or at least $6.3\times{10}^{-4}$ mol of guanine. If we add this to 25 mL of 0.12 M HCl, then there are at least $3.0\times{10}^{-3} - 6.3\times{10}^{-4} = 2.4\times{10}^{-3}$ mol HCl in excess. Titrating with 0.10 M NaOH will require no more than approximately 25 mL to reach the equivalence point. Bromothymol blue is a suitable indicator for this titration.

The percentage of guanine in the sample is

$$ \%\textrm{purity} = \frac { \textrm{mol guanine} }{ \textrm{g sample} } \times 100 $$

$$ \%\textrm{purity} =\frac { \left( \textrm{mol HCl} - \textrm{mol NaOH} \right)  }{ \textrm{g sample} } \times 100 $$

$$ \%\textrm{purity} =\frac { { M }_{ \textrm{HCl} }{ V }_{ \textrm{HCl} }-{ M }_{ \textrm{NaOH} }{ V }_{ \textrm{NaOH} } }{ \textrm{g sample} }\times 100 $$

**Problem 2.** The molar concentration of acetic acid in vinegar is between 0.67 M and 1.33 M. If we dilute 10 mL of vinegar to 50 mL---which gives a molarity between 0.13 M and 0.26 M---and take a 25 mL sample, then titrating with 0.20 M NaOH will require between 17 and 34 mL to reach the equivalence point. Using acetic acid's p${K}_{a}$ of 4.76 and the values above to model the titration curve suggests that the second end point for thymol blue is a suitable indicator.
...
where FW is the formula weight for acetic acid. Given the initial dilution of 10 mL to 50 mL, the actual concentration of ${\textrm{CH}}_{3}\textrm{COOH}$ in the original sample of vinegar is $5\times$ greater.

**Problem 3.** The molar concentration of ammonia in the original product approximately is between 3 M and 6 M. If we dilute 10 mL to 250 mL---which gives a molarity of roughly 0.12 M to 0.24 M---and take a 25 mL sample, then titrating with 0.20 M NaOH will require between 15 mL and 30 mL to reach the equivalence point. Using ammonia's p${K}_{b}$ of 4.76 and the values above to model the titration curve suggests that methyl red is a suitable indicator.
...
$$  \% {\textrm{w/v NH}}_{3}= \frac{\textrm{mol NaOH}\times\textrm{FW}} {{V}_{\textrm{sample}}} = \frac{{M}_{\textrm{NaOH}}{V}_{\textrm{NaOH}}\times\textrm{FW}} {{V}_{\textrm{sample}}} $$

where FW is the formula weight for ammonia. Given the initial dilution of 10 mL to 250 mL, the actual concentration of ${\textrm{NH}}_{3}$ in the original sample of cleaner is $25\times$ greater.

**Problem 4.** With p${K}_{a}$ values of 3.04 and 4.37, students will find that the titration curve has but one clear end point for the titration of tartaric acid's second ${\textrm{H}}^{+}$. Given its range of purity, dissolving 1 g of sample in 100 mL gives a solution that is between 0.054 M and 0.064 M in tartaric acid; titrating a 25 mL portion of this solution with 0.10 M NaOH will require about 26--30 mL to reach the cresol red endpoint.

Knowing that $\textrm{mol tartaric acid} = 0.5\times\textrm{mol NaOH}$ at the equivalence point, the percentage of tartaric acid in the sample is
...
**Problem 5.** With p${K}_{b}$ values of 3.67 and 7.65, students will find that the titration curve has two clear end points, although the second one has a slightly larger $\Delta$pH. Given its range of purity, dissolving 2 g of sample in 100 mL gives a solution that is between 0.057 M and 0.075 M in ${\textrm{CO}}_{3}^{2-}$; titrating a 20 mL portion of this solution with 0.10 M NaOH will require about 22--30 mL to reach the second equivalence point using bromophenol blue to signal the end point.

Knowing that ${\textrm{mol Na}}_{2}{\textrm{CO}}_{3} = 0.5\times\textrm{mol HCl}$ at the equivalence point, the percentage of ${\textrm{Na}}_{2}{\textrm{CO}}_{3}$ in the sample is
...
$$ \%\textrm{w/w}= \frac{0.5 \times \textrm{mol NaOH} \times \textrm{FW}} {\textrm{g sample}\times0.20}\times 100 =  \frac{0.5 \times {M}_{\textrm{NaOH}} {V}_{\textrm{NaOH}} \times \textrm{FW}} {\textrm{g sample}\times0.20}\times 100$$
...
**Problem 6.** With a p${K}_{a}$ values of 4.76 for acetic acid and of 9.24 for the ammonium ion, students will find that both analytes have suitable equivalence points with bromothymol blue serving as a suitable indicator for the titration of acetic acid and alizarin yellow R serving as a suitable indicator for the titration of the ammonium ion. As these titrations must be completed separately, students can tailor the conditions for each analyte.

For the titration of acetic acid, diluting 50 mL of sample to 100 mL brings the concentration, now at 0.10--0.20 M, within the limits imposed by the simulation. Titrating a 25 mL sample using 0.20 M NaOH will require 12--20 mL to reach the end point. For the titration of the ammonium ion, diluting 25 mL of sample to 100 mL brings its concentration into the range 0.125--0.175 M (and brings the concentration of acetic acid into the range 0.05--0.08 M). Titrating a 25 mL sample using 0.20 M NaOH will require 22--35 mL to reach the alizarin yellow R end point.
...
Given the initial dilution of 50 mL to 100 mL, the actual concentration of acetic acid in the original sample is $2\times$ greater.
...
where ${M}_{\textrm{AA}}$ is the molar concentration of acetic acid in the original sample and the factor of 0.25 accounts for the fact that we diluted 25 mL of the original solution to 100 mL. This concentration of ${\textrm{NH}}_{4}^{+}$ is that in the sample as analyzed; given that we diluted 25 mL of the original sample to 100 mL, the actual concentration is $4\times$ greater.